...
class SwooleServer {
    ...
    public function onWorkerStart($serv, $workerId) {
        ...
        // 定义应用目录
        define('APP_PATH', SWOOLE_PATH . DIRECTORY_SEPARATOR . 'App' . DIRECTORY_SEPARATOR);
        // 定义应用模式
        define('APP_MODE', 'cli');
        // 引入ThinkPHP入口文件
	...
	// 这里需要根据不同的标识执行不同的任务	
	if( $data == 'begin'){
            $data = $this->redis->lpop('cTempTask');
            if(!$data){
                return;
            }
        }
        ...
        if( $execdata['crid'] ){
            $jMsg_id = $this->sendTemplateMsg( $execdata );
            
            $next = $this->redis->lpop('cTempTask');
            if($next){
